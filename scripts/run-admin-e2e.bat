...
echo [STEP] Running admin E2E on Chromium...
pushd "%CLIENT_DIR%"
set PLAYWRIGHT_BASE_URL=%FRONTEND_URL%
call npm run e2e -- --project=chromium-desktop tests/e2e/admin-site.spec.ts
set E2E_EXIT=%ERRORLEVEL%
popd

if not "%E2E_EXIT%"=="0" (
    echo [ERROR] Admin E2E failed with exit code %E2E_EXIT%.
    exit /b %E2E_EXIT%
)

echo [DONE] Admin E2E finished successfully.
exit /b 0
